...
* <Rating>-53</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE SLV.S.AML.ALERT.TCIB.REM(GROUP.TXN,ACCT.NO,CUSTOMER.NO,AMOUNT,ADDIT.INFO,RESULT,ADDIT.MSG)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
*
* Nombre: SLV.S.AML.ALERT.TCIB.REM
* Descripción: Rutina para Lanzar Alerta TCIB para Remitentes
*
*-----------------------------------------------------------------------------
...
;*------------------------------------
    Y.CANT = R.COUNTER<EB.SLV63.NUM.REM>
    IF Y.CANT EQ '' THEN
        Y.CANT = 0
    END
...
;*------------------------------------------------------------------------------------------------------
    IF Y.CANT GE Y.LIMITE AND Y.FT.TYPE EQ 'ACAT' AND Y.DEST.CUST EQ CUSTOMER.NO AND FLG.PROCEDER EQ 1 THEN
        RESULT = 'S'
    END
    RETURN
