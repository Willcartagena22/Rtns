...
RETURN
	...
	INIT:
		...
		F.LOCK  = ''
		FN.FAV  = 'FBNK.EB.SLV.STM.FAVORITES'
		F.FAV   = ''
		FN.ERR  = 'F.EB.ERROR'
		F.ERR   = ''	
		FN.ACC  = 'F.ACCOUNT'
		...
		FN.USER = 'F.USER'
		F.USER  = ''
		FN.ITEMS.MAZUL  = 'FBNK.EB.SLV.ITEMS.MONEDA.AZUL'
		F.ITEMS.MAZUL   = ''
		FN.MAS.MAZUL  	= 'FBNK.EB.SLV.MASTER.MONEDA.AZUL'
		F.MAS.MAZUL   	= ''
		Y.PROCESAR = 'N'
...
*	    END
	...
	PROCESS:
			...
			SELECT.CMD = "SELECT " : FN.ITEMS.MAZUL : " WITH MOVIL EQ " : TELEFONONEW : " AND ESTADO EQ AUTORIZADO"
	...
	VALIDAR.LOCK.TOKEN:
		...
		Y.SEEDS  = TELEFONONEW : VM : R.FAV<EB.SLV83.DOCUMENT> : VM
	...
	*-----------------------------------------------------------------------------	
		...
		Y.SEEDS  = TELEFONONEW  : VM : R.ITEMS<EB.IMA.DOCUMENTO> : VM
		Y.SEEDS := Y.FECHA.FROM : VM : Y.TRAN.TIME
		TEXTO.ARCHIVO='Y.SEEDS VALID.TOKEN ':Y.SEEDS
		GOSUB ESCRIBIR.ARCHIVO
		CALL SLV.UTIL.STM.TOKEN(Y.ID.TOKEN, Y.SEEDS, Y.TOKEN.VERIFY, Y.ERROR.TOKEN)
		CRT 'TOKEN: ':Y.TOKEN.VERIFY
	...
	MONEDA.AZUL.INDIVIDUAL:
			;*Moneda Azul Individual
					;*Buscar Favorito a partir del Numero de Telefono
				STMT.FAVORITOS = "SELECT " : FN.FAV : " WITH PHONE EQ " : TELEFONONEW : " AND STATUS EQ ACTIVO"
...
DESENCRIPTAR:
		...
		TELEFONONEW=CEL1:CEL2:CEL3:CEL4:CEL5:CEL6:CEL7:CEL8
...
ESCRIBIR.ARCHIVO:
    ...
    R.ID   = 'SLV.E.STM.INFO.PE.':TODAY:'.txt'
