...
*'FT171377T793'='FT16204C63JJ' 
*'FT171377T793'='CURR.10000000042968.0000017'
 ;*'FT171377T793' = 'CHG1521608938'
	S_DOCUMENT_RNEW	=	'FACTURA'

	N_ACCOUNT_NO 	= 	''
	
    T_APP			=	'FT'
    
    CALL F.READ(FN_FUNDS_TRANS, TRANSACCION, R.ACC, F_FUNDS_TRANS, ERR.ACC)
    ;*IF T_APP EQ 'CH' THEN
    ;*N_ACCOUNT_NO = R.NEW(CHG.DEBIT.ACCOUNT)
    ;*END
    
   ;*Id para parametro de nombre de comision siempre existe para administracion
   ID_CODE_CHARGE	= R.ACC<FT.LOCAL.REF><1,LOCPOSCodeCom>
...
END ELSE   
	...
	*=========================================================================================
	    IF T_APP EQ A_TYPE_APP<2> THEN  
	    	...
	    	CODIGO_CLIENTE		= R.ACC<FT.DEBIT.CUSTOMER>								;*codigo de cliente    	
	   		DESCRIPCION			= R.ACC<FT.ORDERING.CUST>	     						;*Concepto de la transacci�n 
	        N_AMOUNT_TOT		= CHANGE(R.ACC<FT.TOTAL.CHARGE.AMOUNT>,'USD','')		;*Monto neto sujeto
	        N_AMOUNT_TOT        = N_AMOUNT_TOT - Y.LIOF.AMT
			N_AMOUNT_NET		= CHANGE(R.ACC<FT.COMMISSION.AMT><1,1>,'USD','') 			;*Monto sin iva		        
		
			N_AMOUNT_RATE 		= CHANGE(R.ACC<FT.TOTAL.TAX.AMOUNT>,'USD','')			;*Monto de impuesto	
			N_AMOUNT_RATE       = N_AMOUNT_RATE - Y.LIOF.AMT
			S_GIRO				= ''													;*Giro economico        
	        S_CODE_COMM			= R.ACC<FT.COMMISSION.TYPE>	 							;*Codigo de comisi�n
	        S_NAME_CUS_OTH		= ''													;*Nombre del cliente
	        S_NRC_OTH			= R.ACC<FT.LOCAL.REF><1,POSNrcFt>						;*NRC        
	        S_NIT_OTH			= ''					  								;*NIT
	        S_TYPE_ENTRY		= A_TYPE_ENTRY<2>       								;*TIPO DE ENTRADA
	        S_DOCUMENT_RNEW		= R.ACC<FT.LOCAL.REF><1,POSTypeDocFT>					;*TIPO DOCUENTO 
	        B_PAY_COMMI			= ''
	        
	      ;*para comisiones por desembolso	        	        
	        	;*Monto total cargo
				AMOUNT_CHARGE	=	R.ACC<FT.LOCAL.REF><1,LOCPOSDferirAmt>
				AMOUNT_COST		=	R.ACC<FT.LOCAL.REF><1,LOCPOSPrductAmt>
	    	...
	    	IF TOTAL_CHARGE NE 0 THEN
	    		;*Extrayendo datos concepto para la comsion en factura
	    			...
	    			IF EXE_GRAV NE 0 THEN
	    						...
	    						ACC_PRESTAMO	=R.ACC<FT.DEBIT.ACCT.NO>
								ARR_ID			=ACC_PRESTAMO
...
;*	   		END
	...
	*=========================================================================================
	   ;* recuperando cliente a traves de la cuenta
	    ...
	    CALL F.READ(FN_CH_CHRG_BAL, 'FT171377T793', R_CH_CHRG_BA, F_CH_CHRG_BAL, E_CH_CHRG_BAL)
	
	    IF 	N_ACCOUNT_NO NE '' AND R_ACCOUNT<AC.CUSTOMER> NE ''  THEN    	    
	    	CODIGO_CLIENTE		= R_ACCOUNT<AC.CUSTOMER>								;*codigo de cliente    	
	   		DESCRIPCION			= 'Emision de chq. ': 'FT171377T793'   						;*Concepto de la transacci�n 
