...
OPENFILES:
;* Variable donde se recuperará la información de la tabla
...
;* CTA ACREDITADA (ABONADA)
	...
	IF Y.TRAN.TYP EQ 'ACAT' OR Y.TRAN.TYP EQ 'ACIB' OR Y.TRAN.TYP EQ 'ACPE' OR Y.TRAN.TYP EQ 'ACRP' OR Y.TRAN.TYP EQ 'ACPT' THEN 
		Y.VOU.DATE.SYS = OCONV(ICONV(A.RECORD<FT.PROCESSING.DATE>,'D'),'D4/E')
		GOSUB SETRECORD.DATE.SYS	
	END
...
RETURN 
;*
;* AC64 labels para Colector MH - a traves de NPE
SETRECORD.COLECTORES.MHNPE:
	Y.VOU.IDEN 	= Y.VOU.IDEN	: @VM :	'NPE'	: @VM :	'MH.REF'			: @VM :	'MH.FECVEN'
	Y.VOU.LABEL = Y.VOU.LABEL	: @VM : 'NPE'	: @VM : 'Referencia MH'		: @VM :	'Fecha de Vencimiento'
	Y.VOY.VALUE = Y.VOY.VALUE	: @VM : NPE		: @VM :	Y.MH.NUM.REF		: @VM :	 Y.MH.FEC.VENCI
RETURN
