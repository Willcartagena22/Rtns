...
* 20150805N            05 Aug 2015     Nishant               Modified the TFS array formation to accomodate the cheques and other balances.
...
* 20172104             21 Abr 2017     JHENRIQUEZ            Se agrego GoSub para modificar el Narrative de las aplicaciones AC.CHARGE.REQUEST por el momento solo
...
*20150716 - S
    ...
    SEL.LIST = YID.LIST
    SEL.LIST = '181876839841415.010001'
...
*** <region name= DESC CARGO.COLECTOR>
        ...
        ELSE IF Y.FT.TXN.TYPE EQ CARGO.COLECTOR AND CODIGO.COLECTOR.PX NE '' AND R.FUNDS.TRANSFER<FT.DEBIT.ACCT.NO> EQ ACCT.NO THEN
           ...
           ;*CRT 'Y.FT.ID > ':Y.FT.ID
           ;*CRT 'ORDERING.CUST > ':ORDERING.CUST
           ;*CRT 'Y.FT.TXN.TYPE > ':Y.FT.TXN.TYPE
...
*** <desc> </desc>
    ...
    ;*CRT '*********************************************************************************************************' 
    ;*CRT 'Y.FT.ID > ':Y.FT.ID
    ;*CRT 'Y.TRANS.NARR.DESC':Y.TRANS.NARR.DESC
    ;*CRT '*********************************************************************************************************' 
...
GET.NARRATIVE.ABONO.COLECTOR:
   ...
   ;*CRT '********************************************************************'
   ;*CRT 'RECORD.ABONO.APP > ':RECORD.ABONO.APP
   ...
   ;*CRT 'NARRATIVE.ABN > ':NARRATIVE.ABN
   ;*CRT '********************************************************************'
RETURN
