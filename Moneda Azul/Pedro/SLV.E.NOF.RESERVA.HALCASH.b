...
* 1.1       psanchez   26.07.18    Se añaden dos campos adicionales: Comision y Monto Total
*----------------------------------------------------------------------------------------------------
$INSERT I_COMMON
$INSERT I_EQUATE
$INSERT I_System
...
;*  Debug
 ;* SMS.CUSTOMERS ='PSANCHEZBI' 
   ...
   CALL GET.LOC.REF('AC.LOCKED.EVENTS','LF.AMT.BLUE',POS.NM)
	    ...
	    IF NO.OF.RECS.LK.EVENTS NE 0 THEN
	         FOR J = 1 TO NO.OF.RECS.LK.EVENTS
                CALL F.READ(FN.LOCKED.EVENTS,ID.AC.LOCKED.EVEN<J>, LOCKED.EVENTS.REC,F.LOCKED.EVENTS, ERR.BI)
	                 ...
	                 Y.COMISION		= LOCKED.EVENTS.REC<AC.LCK.LOCAL.REF><1,POS.NM>
	                 DEBIT.ACCOUNT  = LOCKED.EVENTS.REC<AC.LCK.ACCOUNT.NUMBER>
	             GOSUB ISNULL
	                 Y.TOTAL		= LOCKED.EVENTS.REC<AC.LCK.LOCKED.AMOUNT>
	                 Y.AMOUNT       = LOCKED.EVENTS.REC<AC.LCK.LOCKED.AMOUNT>  - Y.COMISION
...
GOSUB CLEAR.VARS
	...
	STR.ARR.TX := FMT(Y.AMOUNT,CURRENCY.FORMAT)	: "*" 
	STR.ARR.TX := Y.OUT.DOCUMENT			: "*" 
	STR.ARR.TX := Y.OUT.FROMDATE			: "*" 
	STR.ARR.TX := Y.OUT.TODATE				: "*"
	STR.ARR.TX := FMT(Y.OUT.COMISION,CURRENCY.FORMAT): "*"
	STR.ARR.TX := FMT(Y.OUT.TOTAL,CURRENCY.FORMAT): "*"
	STR.ARR.TX := DEBIT.ACCOUNT
