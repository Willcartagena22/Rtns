...
* <Rating>14173</Rating>
*----------------------------------------------------------------------------------------------------
* Nombre: 		SLV.E.NOF.TXN.CAJERO.DIA
* Descripcion: 	Transacciones del día - cajero.
* EQUIRY:		SLV.E.TXT.CAJERO.DIA
*----------------------------------------------------------------------------------------------------
* Version	Autor		Fecha		Comentario
*----------------------------------------------------------------------------------------------------
* V1		Jonas		24.03.15	Version inicial.
* V2		Jonas		27.03.15	Obtener saldo inical segun ultimo periodo cerrado.
* V4		Jonas		14.04.15	Definicion de grupos por transaccion y categoría. 
* V5		Jonas		04.05.15	Revisión de registro de transacciones y agrupamiento.
* 1.0		Jonas		14.07.15	Agregar transacciones de cajero que no se muestran.
* 2.0		Jonas		22.07.15	Revisión general por duplicidad de registros.
* 3.0		Jonas 		19.04.16	Agregar registros de pago masivo de planillas y prestamos.
* 4.0		Jonas		23.01.17	Revision de descripciones por transacciones LIOF.
* 5.0       Javier      31.03.17    Se cambia INPUTTER de FT por TFS en validacion cambio por eliminacion de WA Fondeo de DAP
* 6.0       CSurio      13.07.17	Mostrar TFS de Colectores Ventanilla por separado.
* 7.0	    Javier		17.10.17	Se agrega Forma de Pago para Transferencias Moviles.
* 8.0	    wrivas		20.07.18	Se agrega Forma de Pago para Transferencias Moviles con descripcion (MonedAzul, Pago MonedAzul).
*-----------------------------------------------------------------------------------------------------------
SUBROUTINE SLV.E.NOF.TXN.CAJERO.DIA(A.INFO)
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
	...
	;*Para mamejo de descripcion de TFS
	Y.TXT.RETIRO  = 'R'
	Y.TXT.DEPOSIT = 'D'
...
*-----------------------------------------------------------------------------
    LOCATE "CAJERO.ID" IN D.FIELDS<1> SETTING POS THEN
    	S.CAJERO.ID = D.RANGE.AND.VALUE<POS>
    END
*-----------------------------------------------------------------------------
* DEBUG
*-----------------------------------------------------------------------------
*S.CAJERO.ID = '4101' ;*'4201' '4191' '4105'
*S.TODAY 	= '20150809' ;*'20151224'
*S.COMPANY   = 'SV0010401'
*-----------------------------------------------------------------------------
;*Obtener la ultima fecha cerrada
CALL F.READ(FN.DATES, S.COMPANY, R.DATES, F.DATES, Y.ERR.R0)
...
*-----------------------------------------------------------------------------
	...
	;*Registro de Cajero
	CALL F.READ(FN.TELL.ID, S.CAJERO.ID, R.TELL.ID, F.TELL.ID, ERR.TELLID)

    ;*Obtener cuenta de cajero
    S.MONEDA 	= "USD"
    S.CATEGORIA = "10001"	;*Cajas
	S.AGENCIA 	= RIGHT(R.TELL.ID<TT.TID.CO.CODE>,4)
    S.INT.CTA 	= S.MONEDA:S.CATEGORIA:S.CAJERO.ID:S.AGENCIA
	S.USER.ID 	= R.TELL.ID<TT.TID.USER>

	;*Agregar registro EB.CONTRACT.BALANCE de saldo inicial
	GOSUB SALDOINI.FIN

	;*Agregar registro TELLER.ID de sobrante/faltante de caja
	...
	;*Agregar registro de TFS Remesas, Depositos y Retiros
	GOSUB TRANSTFS

	;*Agregar registro de AC.CHARGE.REQUEST Cargos - Comisiones
	GOSUB TRANSCHARGES
	
	;*Agregar registro de TELLER
	GOSUB TRANSTELL
...
*	GOSUB TRANSCHEQ	;*Se mueva para evitar duplicidad de registros
...
*-----------------------------------------------------------------------------		    
	...
	S.TIPO.APP = "01"
	S.COD.GRP  = "000000"
	S.DES.TRA = "Saldo Inicial"

	;*Obtener informacion de la cuenta de cajero
	CALL F.READ(FN.ACC, S.INT.CTA, R.ACC, F.ACC, ERR.ACC)
	
	;*Obtener el saldo incial de cajero del ultimo dia cerrado
	CALL EB.GET.ACCT.BALANCE(S.INT.CTA, R.ACC,'BOOKING', S.LAST.PERIOD.END,'', BALANCE, CREDIT.MVMT, DEBIT.MVMT, ERR.MSG)

	MON.INI = ABS(BALANCE)
	S.KEY   = S.TIPO.APP:S.COD.GRP:S.DES.TRA
	
	STR.M.INI = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":MON.INI
	
	A.INFO<-1> 	 = STR.M.INI
	STR.ARR2<-1> = S.KEY

	;*Obtener el saldo final cajero
	MON.FIN = 0.00
	S.TIPO.APP = "05"
	S.COD.GRP  = "110000" 
	S.DES.TRA = "Saldo Final"
 
	;*Obtener el saldo final de cajero del ultimo dia cerrado	
	CALL EB.GET.ACCT.BALANCE(S.INT.CTA, R.ACC,'BOOKING', S.TODAY,'', BALANCE, CREDIT.MVMT, DEBIT.MVMT, ERR.MSG)

	MON.FIN = ABS(BALANCE)
	S.KEY	= S.TIPO.APP:S.COD.GRP:S.DES.TRA

	STR.M.FIN = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":MON.FIN
...
*-----------------------------------------------------------------------------		    
	...
	IF ABS(S.MON.DIF) GT 0 THEN
		...
		S.KEY  	= S.TIPO.APP:S.COD.GRP:S.DES.TRA 
		STR.DIF = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":ABS(S.MON.DIF)
...
TRANSFT.PRE.DAP:
*-----------------------------------------------------------------------------		    
*	SELECT.FT.PD  = "SELECT ": FN.FT:" WITH (CO.CODE EQ '":S.COMPANY:"') AND (PROCESSING.DATE EQ '": S.TODAY :"') AND (TRANSACTION.TYPE EQ 'ACDF' OR TRANSACTION.TYPE EQ 'ACRP')"
...
*	SELECT.FT.PD := " AND @ID EQ 'FT17049ZBB0M;1'"	 
	...
	FOR I = 1 TO NO.OF.RECS
        ...
        CALL F.READ(FN.FT, LIST.FT.PD<I>, R.FT.PD, F.FT, Y.ERR.FTPD)
		...
		;*Se cambia INPUTTER de FT por TFS ya que la FT viaja por OFS cambio por 
		;*desarrollo eliminacion de WA Fondeo de DAP : OCORNEJO 31.03.2017
		;*----------------------------------------------------------------------
		IF S.REG.VALIDO EQ 'S' AND R.FT.PD<FT.DEBIT.ACCT.NO> EQ S.ACC.DEP AND FIELD(R.TFS.PD<TFS.INPUTTER>,"_",2) EQ S.USER.ID AND Y.TFS.CO.CODE EQ S.COMPANY THEN
			STR.REGIS<-1> =  Y.ORDER.CUS:"*":LIST.FT.PD<I>
			S.TIPO.APP  = "01"
			...
			S.KEY = S.TIPO.APP:S.COD.GRP:S.DES.TRA
			STR.FT.PD = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":N.MONTO
			Y.MON.REG = N.MONTO
			STR.DATO = STR.FT.PD
...
TRANSTFS.CNR:
*-----------------------------------------------------------------------------		    
	SELECT.TFS.CNR  = "SELECT " : FN.TFS : " WITH (CO.CODE EQ '": S.COMPANY :"') AND (BOOKING.DATE EQ '":S.TODAY:"')"
	...
	FOR I = 1 TO NO.OF.RECS
		...
		IF S.COD.CAJERO EQ S.CAJERO.ID AND S.REG.VALIDO EQ "S" AND Y.POS EQ 0 THEN
			...
			IF S.MONTO.CNR GT 0 THEN
				...
				STR.CNR = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":TRIM(S.DES.TRA):"*":S.INT.CTA:"*":1:"*":S.MONTO.CNR
...
RETURN
*-----------------------------------------------------------------------------
;*Evaluar los TFS para deposito y retiros en cuenta de ahorro y corriente.
TRANSTFS:
*-----------------------------------------------------------------------------		    
	SELECT.TFS  = "SELECT " : FN.TFS : " WITH (CO.CODE EQ '": S.COMPANY :"') AND (BOOKING.DATE EQ '":S.TODAY:"')"
	;*SELECT.TFS  := " AND @ID EQ 'TFS15221HP1MT'"
    ...
    WHILE TFS.ID:POS01
		...
		;*Evaluar si el TFS ya fue agregado
		...
		IF Y.POS EQ 0 THEN
			...
			;*Obtener código de cajero de TELLER
			CALL F.READ(FN.TELLER, FIELD(R.TFS<TFS.UNDERLYING>, VM, 1), R.TELLER, F.TELLER, Y.ERR.TELL)
	
			S.COD.CAJERO = R.TELLER<TT.TE.TELLER.ID.1>
			
			IF S.COD.CAJERO EQ S.CAJERO.ID AND S.REG.VALIDO = 'S' THEN
...
*					Y.CUENTA = RIGHT(FIELD(R.TFS<TFS.ACCOUNT.DR>, VM, 1), 8)
...
*							Y.TIPO.TRANS = R.GEN.PARAM<SLV.GEN.TX.TYPE.PARAM><Vp1,1>
						...
						FOR J = 1 TO S.NO.REG.REF
							...
							IF Y.MONTO.TOTAL GT 0 THEN
								...
								;*Para Acumular estas Transacciones en el Mismo Registro (con descripcion MonedAzul) Sin Distincion de Tipo de Cuenta : WRIVAS 20.07.2018
								IF R.TFS<TFS.TRANSACTION><1, J> EQ 'PagoMonedAzul' THEN
									S.COD.GRP = 'COD01'
...
*										Y.DES.CTA.PRO = Y.DES.TRANS:" ":Y.CTA.PRO
...
*										Y.DES.CTA.TER = Y.DES.TRANS:" ":Y.CTA.TER
								...
								IF Y.MON.CTA.PRO GT 0 THEN
									...
									STR.TELL = S.KEY:"*":S.TIPO.APP.CTA:"*":S.COD.GRP:"*":Y.DES.CTA.PRO:" ":'':"*":S.INT.CTA:"*":1:"*":Y.MON.CTA.PRO
								...
								IF Y.MON.CTA.TER GT 0 THEN
									...
									STR.TELL = S.KEY:"*":S.TIPO.APP.CTA:"*":S.COD.GRP:"*":Y.DES.CTA.TER:" ":'':"*":S.INT.CTA:"*":1:"*":Y.MON.CTA.TER
						...
						STR.TELL = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":TRIM(S.DES.TRA):" ":TRIM(S.DES.GRP):"*":S.INT.CTA:"*":1:"*":S.MONTO.TFS
...
TRANSCHARGES:
*-----------------------------------------------------------------------------		    
	SELECT.ACCHR  = "SELECT " : FN.ACCHR : " WITH CO.CODE EQ '": S.COMPANY :"' AND CHARGE.DATE EQ '":S.TODAY:"' AND STATUS EQ 'PAID'"
	...
	FOR I = 1 TO NO.OF.RECS
		...
		IF FIELD(R.ACCHR<CHG.INPUTTER>,"_",2) EQ S.USER.ID AND S.REG.VALIDO EQ 'S' THEN
			;*Extraer informacion de AC.CHARGE.REQUEST
			...
			S.KEY = S.TIPO.APP:S.COD.GRP:S.DES.TRA
			STR.ACCHR = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":TRIM(S.DES.TRA):"*":S.INT.CTA:"*":1:"*":S.MONTO.ACCHR
...
TRANSTELL:
*-----------------------------------------------------------------------------		    
	SELECT.TELL  = "SELECT " : FN.TELLER : " WITH CO.CODE EQ '": S.COMPANY :"' AND (TELLER.ID.1 EQ '":S.CAJERO.ID:"' OR TELLER.ID.2 EQ '":S.CAJERO.ID:"') AND VALUE.DATE.1 EQ '":S.TODAY:"'"
			...
			IF FIELD(R.TELLER<TT.TE.NARRATIVE.1>, SM, 4) EQ '' THEN
				...
				IF S.REG.VALIDO EQ 'S' THEN
					...
					IF R.TELLER<TT.TE.DR.CR.MARKER> EQ 'DEBIT' THEN
						IF CHANGE(R.TELLER<TT.TE.ACCOUNT.1>,"-","") EQ S.INT.CTA THEN
							S.TIPO.APP = "01"
						END ELSE
							S.TIPO.APP = "02"
						END					
					...
					IF CHANGE(R.TELLER<TT.TE.ACCOUNT.1>,"-","") NE S.INT.CTA AND CHANGE(R.TELLER<TT.TE.ACCOUNT.2>,"-","") NE S.INT.CTA THEN
						S.TIPO.APP = "03"
					END
					
					;*Consideración para transacción de cambio de moneda
					IF CHANGE(R.TELLER<TT.TE.ACCOUNT.1>,"-","") EQ S.INT.CTA AND CHANGE(R.TELLER<TT.TE.ACCOUNT.2>,"-","") EQ S.INT.CTA THEN
						S.TIPO.APP = "03"
					END 
					...
					IF R.TELLER<TT.TE.TRANSACTION.CODE> EQ '26' THEN
						...
						;*Evaluar si la cuenta esta en historicoc
						IF DCOUNT(R.ACC, "?") EQ 0 THEN
							;*Extraer informacion de ACCOUNT$HIS
					...
					STR.TELL = S.KEY:"*":S.TIPO.APP:"*":R.TELLER<TT.TE.TRANSACTION.CODE>:"*":Y.DESC.TT.TXN:" ":S.DES.GRP:"*":S.INT.CTA:"*":"1":"*":R.TELLER<TT.TE.AMOUNT.LOCAL.1>
...
TRANSFT:
*-----------------------------------------------------------------------------		    
	SELECT.FT  = "SELECT ": FN.FT:" WITH CO.CODE EQ ":S.COMPANY:" AND PROCESSING.DATE EQ '": S.TODAY :"' AND (TRANSACTION.TYPE NE 'ACDF' AND TRANSACTION.TYPE NE 'ACRP')"
	...
	FOR I = 1 TO NO.OF.RECS
		...
		;*Validar si transaccion ya fue procesada
		...
		IF Y.POS EQ 0 AND Y.POS.TFS EQ 0 THEN 
			...
			IF S.REG.VALIDO EQ 'S' THEN
				...
				IF CHANGE(R.FT<FT.DEBIT.ACCT.NO>,"-","") EQ S.INT.CTA OR CHANGE(R.FT<FT.CREDIT.ACCT.NO>,"-","") EQ S.INT.CTA THEN	
					N.MONTO	   = R.FT<FT.LOC.AMT.DEBITED>			
					
					IF CHANGE(R.FT<FT.DEBIT.ACCT.NO>,"-","") EQ S.INT.CTA THEN
						S.TIPO.APP = "01"
						N.MONTO	   = R.FT<FT.LOC.AMT.DEBITED>
					END
	
					IF CHANGE(R.FT<FT.CREDIT.ACCT.NO>,"-","") EQ S.INT.CTA THEN
						S.TIPO.APP = "02"
						N.MONTO	   = R.FT<FT.LOC.AMT.CREDITED>
					END
					
					S.KEY = S.TIPO.APP:S.COD.GRP:S.DES.TRA
					STR.FT = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":N.MONTO
					Y.MON.REG = N.MONTO
					STR.DATO = STR.FT 
					GOSUB REGISTRO.TRANS
				END
				...
				IF CHANGE(R.FT<FT.DEBIT.ACCT.NO>,"-","") NE S.INT.CTA AND CHANGE(R.FT<FT.CREDIT.ACCT.NO>,"-","") NE S.INT.CTA AND FIELD(R.FT<FT.INPUTTER>,"_",2) EQ S.USER.ID THEN
					S.TIPO.APP = "03"
					N.MONTO	   = R.FT<FT.LOC.AMT.DEBITED>			
					
					S.KEY = S.TIPO.APP:S.COD.GRP:S.DES.TRA
					STR.FT.OTR = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":N.MONTO 
					Y.MON.REG = N.MONTO
					STR.DATO = STR.FT.OTR
					GOSUB REGISTRO.TRANS
				END
...
TRANSCHEQ:
*-----------------------------------------------------------------------------		    
	SELECT.TELL  = "SELECT " : FN.TELLER : " WITH CO.CODE EQ '": S.COMPANY :"' AND TELLER.ID.1 EQ '":S.CAJERO.ID:"' AND VALUE.DATE.1 EQ '":S.TODAY:"'"
	...
	FOR I = 1 TO NO.OF.RECS
		...
		IF S.REG.VALIDO EQ 'S' AND (R.TELLER<TT.TE.STOCK.NUMBER> GT 0 OR R.TELLER<TT.TE.CHEQUE.NUMBER> GT 0) THEN
			...
			S.KEY = S.TIPO.APP:S.COD.GRP:S.DES.TRA
			STR.TELL = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":S.DES.TRA:"*":S.INT.CTA:"*":"1":"*":R.TELLER<TT.TE.AMOUNT.LOCAL.1> 
			Y.MON.REG = R.TELLER<TT.TE.AMOUNT.LOCAL.1>
			STR.DATO = STR.TELL
			GOSUB REGISTRO.TRANS
		END
	...
	IF Y.MON.REG GT 0 THEN
		...
		IF Y.POS EQ 1 THEN
			Y.ARR.POS 	= A.INFO<Ap1, Vp1>
		  	Y.NO.REC 	= FIELD(Y.ARR.POS,"*", 6) + 1
		  	Y.MONTO 	= FIELD(Y.ARR.POS,"*", 7)
		  	Y.TOT.MONTO = Y.MONTO + Y.MON.REG
		  	A.INFO<Ap1, Vp1> = FIELD(Y.ARR.POS,"*", 1):"*":FIELD(Y.ARR.POS,"*", 2):"*":FIELD(Y.ARR.POS,"*", 3):"*":FIELD(Y.ARR.POS,"*", 4):"*":FIELD(Y.ARR.POS,"*", 5):"*":Y.NO.REC:"*":Y.TOT.MONTO
		END ELSE
			A.INFO<-1> 	 = STR.DATO
			STR.ARR2<-1> = S.KEY
		END
...
*-----------------------------------------------------------------------------
	...
	IF Y.MON.REG GT 0 THEN
		Y.IMPUESTO = 1
		S.KEY = S.TIPO.APP.CTA:S.COD.GRP:Y.DES.CTA.PRO ;*:S.DES.GRP
		STR.DATO = S.KEY:"*":S.TIPO.APP.CTA:"*":S.COD.GRP:"*":Y.DES.CTA.PRO:" ":S.DES.GRP:"*":S.INT.CTA:"*":1:"*":Y.MON.REG
		GOSUB REGISTRO.TRANS
...
*							Y.DES.CTA.PRO = Y.DES.CTA.CHQ:" ":Y.DESC.PARAM
							Y.DES.CTA.PRO = Y.DES.CTA.CHQ:" ":S.DESC.CHQ
						END
					END
				END
				...
				IF Y.MON.CTA.PRO GT 0 THEN
					S.KEY = S.TIPO.APP.CTA:S.COD.GRP:Y.DES.CTA.PRO ;*:S.DES.GRP
					STR.DATO = S.KEY:"*":S.TIPO.APP.CTA:"*":S.COD.GRP:"*":Y.DES.CTA.PRO:" ":'':"*":S.INT.CTA:"*":1:"*":Y.MON.CTA.PRO
					Y.MON.REG = Y.MON.CTA.PRO
					GOSUB REGISTRO.TRANS
				END
				IF Y.MON.CTA.TER GT 0 THEN
					S.KEY = S.TIPO.APP.CTA:S.COD.GRP:Y.DES.CTA.TER ;*:S.DES.GRP
					STR.DATO = S.KEY:"*":S.TIPO.APP.CTA:"*":S.COD.GRP:"*":Y.DES.CTA.TER:" ":'':"*":S.INT.CTA:"*":1:"*":Y.MON.CTA.TER
					Y.MON.REG = Y.MON.CTA.TER
					GOSUB REGISTRO.TRANS
				END
			END
...
;*----------------------------------------------------------------------------    
    ...
    Y.ID.ACCOUNT.PARAM = S.COMPANY : '.' : Y.ID.COLECTOR.PEX : '.VEN'
    ...
    ;*CRT 'S.COMPANY      = ' : S.COMPANY
    ;*CRT 'S.CTA.COLECTOR = ' : S.CTA.COLECTOR
    ;*CRT 'S.TODAY        = ' : S.TODAY
    
    IF S.CTA.COLECTOR NE '' THEN    
        Y.TFS.SELECT   = "SELECT " : FN.TFS
        Y.TFS.SELECT  := "  WITH CO.CODE         EQ " : "'" : S.COMPANY      : "'"    
        ...
        WHILE Y.TFS.ITEM  NE ''
		    ...
		    IF S.COD.CAJERO EQ S.CAJERO.ID AND S.REG.VALIDO EQ "S" AND Y.POS EQ 0 THEN 		    
			    ...
			    ;*CRT 'TFS = ' : Y.ID.TFS : '  MONTO = ' : S.MONTO.TFS
			    
			    IF S.MONTO.COLECTORES GT 0 THEN
				    S.KEY          = S.TIPO.APP:S.COD.GRP:S.DES.GRP				    
				    STR.COLECTORES = S.KEY:"*":S.TIPO.APP:"*":S.COD.GRP:"*":TRIM(S.DES.TRA):"*":S.INT.CTA:"*":1:"*":S.MONTO.COLECTORES
