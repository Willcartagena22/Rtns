...
;*CONSULTAMOS TODAS LAS AGENCIAS DE BANCO AZUL
    ...
    FOR I=1 TO NO.OF.RECS.AGENCIAS
        ...
        ;*STMT PARA RUTINA DE ABONO AUTOMATICO
             STMT.PAGOS.DIA = "SELECT ":APPLICATION.RS:" WITH FECHA EQ '":FECHA.TRX:"' AND CMP.RESERVA.11 EQ 'CONCILIADO' AND CMP.RESERVA.8 EQ 'PENDIENTE'" 
    ...
    ;*Liquidacion Automatica de Colectores Ventanilla
    ;*-----------------------------------------------
    CALL SLV.V.LIQ.CTAS.PEX.VEN
...
UPDATE.STATUS.RECORD.ABONO:
    GOSUB GET.DATE.SYSTEM
    ;*FECHA.TRX = '20160927'
    STMT.ARRANGEMENT = "SELECT ":APPLICATION.NAME:" WITH FECHA EQ ":FECHA.TRX:" AND CMP.RESERVA.11 EQ 'CONCILIADO' AND CMP.RESERVA.8 EQ 'PENDIENTE'"
    ;*STMT.ARRANGEMENT = "SELECT ":APPLICATION.NAME:" WITH FECHA EQ ":FECHA.TRX:" AND CMP.RESERVA.11 EQ 'ABONADO' AND CMP.RESERVA.8 EQ 'PENDIENTE'"
    ...
    FOR G=1 TO NO.OF.RECS
        ...
        CALL SLV.V.ACTUALIZACION.ESTADOS(ID.TRX,'ABONO')
...
SEND.OFS.PREVIEW.ACC.PEX:
    ...
    TRANS.ID = ''
    ORDER.CUST  = 'ABONO.CUENTA.CL'
    ORDER.BANK  = 'ABONO.CUENTA.CL'
	R.FT = ''
	R.FT<FT.DEBIT.ACCT.NO>    =  ACCOUNT.BRANCH
*	R.FT<FT.DEBIT.AMOUNT>     =  Y.AMOUNT
 	R.FT<FT.CREDIT.ACCT.NO>   =  CUENTA.TRANSITORIA.CL
	R.FT<FT.DEBIT.CURRENCY>   =  'USD'
	R.FT<FT.DEBIT.AMOUNT>     =  MONTO.OFS.PREVIEW
...
PROCCESS.ABONO.ACCOUNT.COLECTOR:
    ...
    ORDER.BANK  = 'ABONO.CLIENTE.CL'
    ID.PARAM.OFS.2 = 'OFS.I.FT.ABONO.COL'
    TRANS.ID.2 = ''
	R.FT.2 = ''
	R.FT.2<FT.DEBIT.ACCT.NO>    =  DEBIT.ACC
*	R.FT<FT.DEBIT.AMOUNT>     =  Y.AMOUNT
 	R.FT.2<FT.CREDIT.ACCT.NO>   =  CREDIT.ACC
	R.FT.2<FT.DEBIT.CURRENCY>   =  'USD'
	R.FT.2<FT.DEBIT.AMOUNT>     =  Y.AMOUNT
	R.FT.2<FT.ORDERING.BANK>    =  ORDER.BANK
	R.FT.2<FT.ORDERING.CUST>    =  ORDER.CUST
	R.FT.2<FT.TRANSACTION.TYPE> =  CODE.TXN
	Y.OUT.2    				    =  ''
    
    CALL SLV.UTIL.OFS.TRX(TRANS.ID.2,R.FT.2,ID.PARAM.OFS.2,Y.OUT.2)
