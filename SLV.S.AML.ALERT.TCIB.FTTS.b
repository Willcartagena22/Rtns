...
* <Rating>-36</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE SLV.S.AML.ALERT.TCIB.FTTS(GROUP.TXN,ACCT.NO,CUSTOMER.NO,AMOUNT,ADDIT.INFO,RESULT,ADDIT.MSG)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
*
* Nombre: SLV.S.AML.ALERT.TCIB.FTTS
* Descripción: Rutina para Lanzar Alerta TCIB Transferencias a Terceros Salientes
*
*-----------------------------------------------------------------------------
...
RETURN
	...
	INIT:
		...
		;*Constante
		EQU ALERT.ID TO 'AL.FTTS'
		
		;*Variables
		Y.FT.TYPE     = R.NEW(FT.TRANSACTION.TYPE)
		Y.CUST.FT.SAL = R.NEW(FT.DEBIT.CUSTOMER)
	RETURN
	...
	PROCESS:    	
    	...
    	;*------------------------------------
    	Y.CANT = R.COUNTER<EB.SLV63.NUM.FT.SAL>
    	IF Y.CANT EQ '' THEN
    		Y.CANT = 0
    	END
    	...
    	;*------------------------------------------------------------------------------------------------------
    	IF Y.CANT GE Y.LIMITE AND Y.FT.TYPE EQ 'ACAT' AND Y.CUST.FT.SAL EQ CUSTOMER.NO THEN
    		RESULT = 'S'
    	END
	RETURN
END
