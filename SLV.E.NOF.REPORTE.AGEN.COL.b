...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE SLV.E.NOF.REPORTE.AGEN.COL(A.INFO)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
PROCCESS:
    ...
    LOCATE "FECHA.INFO" IN D.FIELDS<1> SETTING POSDATE THEN
...
S.DATE.FIND= '201609'
    ...
    IF S.COLECTOR.ID NE '' THEN
        FOR K=1 TO DCOUNT(ID.COLECTOR.DETALLE,FM)
            ...
            IF S.COLECTOR.ID EQ ID.COLECTOR.DETALLE<K> THEN
                ...
                STMT.ARRANGEMENT = "SELECT ":FN.TABLA.PAGO.COL:" WITH @ID LIKE %":ID.BUSQUEDA:"% AND ID.COLECTOR EQ ":S.COLECTOR.ID:"AND AGENCIA EQ":AGENCIA
    ...
    IF S.COLECTOR.ID EQ '' THEN
        ...
        FOR K=1 TO DCOUNT(ID.COLECTOR.DETALLE,FM)
            ...
            STMT.ARRANGEMENT = "SELECT ":FN.TABLA.PAGO.COL:" WITH @ID LIKE %":ID.BUSQUEDA:"% AND ID.COLECTOR EQ ":ID.COLECTOR.DETALLE<K> :" AND AGENCIA EQ":AGENCIA
            ...
            FOR I=1 TO NO.OF.RECS
                ...
                COLECTOR    = NOMBRE.COLECTOR
                FECHA.HR    = R.FUNDS.TRANSFER<FT.DATE.TIME>
                MONTO.FR    = RS.PAGO.COL<EB.SLV54.MONTO>
                AGEN.FRM    = NOMBRE.AGENCIA
                USR         = USUARIO.TRX
                REF.FRM     = ID.TRX.COL
                NO.CONF     = NO.CONF.PEX
