...
;*definicion de tipo de documento por condiciones de cliente(para los casos en los que no se pueda definir documento siempre sera factura)
    N_TYPE_DOC			=	N_ISFCF
        ...
        IF COLECTOR EQ '03-BSeguros' AND T_APP EQ A_TYPE_APP<4> THEN
        N_TYPE_DOC			=	N_ISFCF
        END
...
;* Validar si es un AC.Charge para MH y cambiar la el detalle de la factura
 IF COLECTOR EQ '03-BSeguros' AND T_APP EQ A_TYPE_APP<4> THEN
        ...
        PARAMETROSEG='DESC.FACT.SEG.AZUL'        
        CALL F.READ(FN_SLVPA, PARAMETROSEG, R.TABLE.PA, F_SLVPA, F.ERR.PA)
        TEXTOSEG = R.TABLE.PA<EB.SLV39.VALOR.PARAM>
        TEXTOSEG1 = R.TABLE.PA<EB.SLV39.DESC.PARAM>
        LINE_CONCEPT='DETALLES!' : S_DESC_COMM : '!' : FMT(NOSUJETA, 'R2') : '!' : FMT(N_VENTA_EXENTA, 'R2') : '!' : FMT(N_VAL_IN_DETA, 'R2') : '!':CHAR(10)
	   	LINE_CONCEPT:= 'DETALLES!': TEXTOSEG1:' ':TEXTOSEG:' ':MES.TODAY:'/':ANIO.TODAY
