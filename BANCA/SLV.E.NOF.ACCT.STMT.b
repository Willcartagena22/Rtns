...
* 20180129             29 Ene 2018     rmoran				 Se realiza cambio para segmentar la consulta desde banca en linea se coloca bandera Y.FLAG.TCIB  para detectar que solicitud es de banca en linea y consultar por rango de limites
* 20180122			   25 Jun 2018     galfaro				 Se agrega evaluacion para campo credito
* 20180809			   09 Agos 2018	   wrivas				 Se anade logica para colocar narrative de txn para pago de Targeta Visa desde Banca en linea
* 20180823			   23 Agos 2018	   rmezquita			 Se anade logica para colocar narrative de Pago de Colectores Manuales
* 20180910			   10 Sep 2018	   iTurcios				 Se modifica logica para enviar a banca solamente los ultimos 150 txn
* 20190206             02 FEB 2019     rortiz                Se agrega descripcion para transacciones AC66 MH NPE/BARRA y ESPECIFICOS
* 20190225             25 FEB 2019     rortiz                Se agrega descripción para transacción ACCA en la aplicación FUNDS.TRANSFER
* 20190427			   27 Abr 2019	   eescobar				 Se modifica para mostrar descripcion de IVA por Comision de moneda azul y liberacion de fondos desde KEYS.PARAMS
* 20190305			   03 May 2019     vburgos				 Integrando MonedAzul
* 20190524			   24 May 2019	   eescobar				 Se modifica para dar tratamiento a transacciones FT reversadas tomando en cuenta Y.STMT.REVERSAL.MARKER
* 20190507			   05 May 2019	   rcortes				 Se agrega la logica para las Transferencias Internacionales ACTI 
* 20190521		       21 May 2019     chernandez			 Integrando cambios CONFIA			 
* 20190610			   11 Jun 2019	   iTurcios				 Se modifica para mostrar descripcion de IVA por Comision de Transferencias Internacionales desde KEYS.PARAMS
* 20191209			   09 Dic 2019	   rcortes				 Se adiciona logica para transacciones PNAE 		
* tk:41458			   23 Oct 2019	   Jonas I.				 No mostrar linea cuando IVA sea cero (por cliente exento)	 
* 20200107			   07 Ene 2020	   elara				 Se corrige la búsqueda en el histórico para prevenir mostrar la tranacción de seguro en el estado de cuenta	 
*--------------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE   
...
*** <desc> </desc>
    ...
    FN.SLV.COLECTORES = 'F.EB.SLV.COLECTORES'
    F.SLV.COLECTORES = ''
   CALL OPF(FN.SLV.COLECTORES, F.SLV.COLECTORES)
   ...
   FN.COMPANY = 'F.COMPANY'
    ...
    EQU TCE.MAX.TXN TO 'SLV.TCE.MAX.TXN.STMT'
    EQU PAGO.CH.PROPIO TO 'PagoChPropio'
    EQU ACRE.CH.AJENO  TO 'AcredChAjeno'
    EQU CM.COLECTOR TO 'CM'
    FLD.NAME  = 'LF.CM.CODCOLEC'
	CALL GET.LOC.REF('TELLER.FINANCIAL.SERVICES',FLD.NAME,FLD.POS)	
    TFS.TIPO.TRANS   = ''
    TFS.TT.TRANS     = ''
    FT.NOTA.DESC     = ''
    ES.CHQ.PRP       = 0
	
	;*Parametrización moneda azul MAPE
	FN.SLV.CANAL.MAPE 	= 'F.EB.SLV.CANAL.MONEDAZUL'
	F.SLV.CANAL.MAPE	= ''
	CALL OPF(FN.SLV.CANAL.MAPE, F.SLV.CANAL.MAPE)
...
;*--------------------------------------------------------------------------------- 
    ...
    EQU TRX.CONVENIOS  TO 'ACCA'
    EQU TRX.GENERICA   TO 'AC'
...
;*	Debug   
;*	ACCT.NO   = '10000000001528' ;*Cuando se llama desde t24  
;*	ACCT.NO   = '10000000008678' ;*Cuando se llama desde t24 
;*	ACCT.NO   = '10000000046955' ;*Cuando se llama desde t24 
;*	ACCT.NO   =	'10000000005388|0';* formado de llamada desde banca en linea CUENTA|0
;*	DATE.FROM = TODAY ;*'20180718';*TODAY;*'20180101';*'20150701';
;*	DATE.TO   = TODAY ;*'20180818';*TODAY ;*'20180131' ;*'20150810' 
;*	ACCT.NO   =	'10000000005949';* formado de llamada desde banca en linea CUENTA|0
;*	DATE.FROM = '20190126';*'20190325';*TODAY;*'20190321';*'20150701';
;*	DATE.TO   = '20190126';*'20190325';*TODAY ;*'20180131' ;*'20150810'
	 
   	Y.CANT.TXN.TCIB = 0
    Y.FLAG.TCIB = 0    
    FINDSTR "|" IN ACCT.NO SETTING Ap,Vp THEN 
        ;*Peticion por banca en linea
    	Y.FLAG.TCIB = 1	;*Flag para detecta peticion desde Banca
    	Y.ACCT.NO	= ACCT.NO ;*Numero Cuenta
    	ACCT.NO 	= FIELD(ACCT.NO,'|',1)	;*0 para detectar que viene de Banca	
		Y.LIMIT.I	= 0 ;*Limite inicial
		Y.LIMIT.F	= 0 ;*Limite final
		
		;*Lectura parametro de cantidad de Txn a devolver a Banca en Linea
		CALL F.READ(FN.EB.SLV.GLOBAL.PARAM, TCE.MAX.TXN , REC.TCE.MAX.TXN, F.EB.SLV.GLOBAL.PARAM, ERR.TCE.MAX.TXN)
		Y.CANT.TXN.TCIB	= REC.TCE.MAX.TXN<EB.SLV39.VALOR.PARAM>
	END  
...
*Variables limites consulta banca en linea
	...
	IF Y.FLAG.TCIB EQ 1 THEN	 
		Y.LIMIT.F = Y.LAST.CNT ;* Ejm: 10		 
		IF Y.LAST.CNT LE Y.CANT.TXN.TCIB THEN				
			Y.LIMIT.I = 1 ;*Empieza ciclo desde posicion 1
		END 
		ELSE
			Y.LIMIT.I = Y.LIMIT.F - Y.CANT.TXN.TCIB	;*Ejm: 850
		END			 
    END  
...
MAIN.PROCESS:
    ...
    END
     ...
     ;*CAMBIOS REALIZADOS CHERNANDEZ
    COD.COLECTOR = R.TFS<TFS.LOCAL.REF><1,FLD.POS>
    NAT.TRANSACTION = R.TRANSACTION<AC.TRA.DEBIT.CREDIT.IND>    
    RECORD.ABONO.APP = 'SLV.COLECTORES.DESC'
    
    IF COD.COLECTOR EQ '04' OR COD.COLECTOR EQ '05' OR COD.COLECTOR EQ '08' AND NAT.TRANSACTION EQ 'DEBIT' THEN 
            CALL F.READ(FN.EB.SLV.KEYS.PARAMS,RECORD.ABONO.APP,RESPONSE.NRR.ABN,F.EB.SLV.KEYS.PARAMS,ERR.APP.KEYS)
            R.FIND = CM.COLECTOR:'.':COD.COLECTOR:'.':'P'
            FIND R.FIND IN RESPONSE.NRR.ABN<EB.SLV18.PARAM.ID> SETTING Ap, Vp THEN
            NARRATIVA.COLEC = CHANGE(RESPONSE.NRR.ABN<EB.SLV18.VALOR><Ap, Vp>, SM, VM)
            Y.TRANS.NARR.DESC = NARRATIVA.COLEC
            END     
    END
...
*
    ...
    IF Y.STMT.TRANS.REFERENCE[1,2] EQ 'FT' THEN
        ...
        ;*Para Validar si debe Ocultarse la Txn es decir si es Retencion de Seguro por Pago Anticipado : OCORNEJO 06.03.2018
        ;*==================================================================================================================
        GOSUB HIDE.UNC.TXN
        IF Y.FLG.HIDE EQ 'Y' THEN
        	RETURN
        END
        ;*==================================================================================================================
    END
...
*Chnages for the CR on 25/08/2015
    IF NARR THEN
    ** Change for JCORTES 20/05/2019
        IF (Y.FT.TXN.TYPE NE 'ACTI') OR (Y.FT.TXN.TYPE EQ 'ACTI' AND Y.FLAG.CLIENTE NE 'Y') THEN
...
;*-----------------------------------------------------------------------------------------
    Y.CAT.DIV.DESC = RECORD.CAT.DIVIDE<EB.SLV18.DESCRIPCION><Ap,Vp>
    IF Y.STMT.AMOUNT.LCY THEN
...
*-----------------------------------------------------------------------------
    ...
    END ELSE
        ...
        END
    END
    	;*Se agrega GOSUB para evaluar si la transaccion es por pago de colector
    	;*::::::::::::::::::::::::::: 
	    GOSUB GET.NARRATIVE.CM
	    ;*PRINT "TT.CODE.TRANS >> ":TFS.TT.TRANS
    END
...
*   DESCRIPCION.IVA.COMI = 'IVA por Liberacion de Fondos'
	DESCRIPCION.IVA.COMI = Y.CAT.DIV.DESC
    Y.APP = SUBSTRINGS(Y.STMT.THEIR.REF,1,3)
    IF Y.TAX.AMT.CHG LT 0 AND Y.APP EQ 'CHG'  AND Y.TAX.AMT.CHG THEN
        Y.TEMP.ARR<-1> = Y.STMT.ID:'*':Y.ACCT.ID:'*':Y.ACCOUNT.NUMBER:'*':Y.STMT.CURRENCY:'*':Y.STMT.COMPANY
...
*wrivas:
*;*Debug:     
*     IF Y.FT.ID EQ "FT152219YQP3" THEN 
*      CRT Y.FT.ID
*     END  
...
* <20191209>        
		Y.ID.CUS		= R.FUNDS.TRANSFER<FT.DEBIT.CUSTOMER>
		CALL F.READ(FN.CUS, Y.ID.CUS, ARR.CUS, F.CUS, ERR)
    	IF ARR.CUS THEN
			Y.CUS.STS 	= ARR.CUS<EB.CUS.CUSTOMER.STATUS> ;* 5 = PNAE
			CALL GET.LOC.REF("CUSTOMER", "SEGMENT", SEG.POS)	
			Y.CUS.SEG	= ARR.CUS<EB.CUS.LOCAL.REF, SEG.POS> ;* 1 = Persona Natural
		END
* </20191209>		
*----------------------------------------------------------------------------------------------------------------------------
*;*wrivas: Logica para armar narrative para transacciones internacionesles desde las versiones de FT ABONO, CARGO, REINTEGRO
    IF Y.FT.TXN.TYPE EQ 'ACTI' THEN
        IF Y.TXN.CODE EQ 234 THEN
            ;*Y.TRANS.NARR.DESC = 'Comision transferencia Cobranzas Importación'
            CALL SLV.NARR.TXN.INTERNACIONAL(ACCT.NO, Y.FT.ID,Y.TXN.CODE ,NARRATIVE, FLAG.CLIENTE)
                Y.TRANS.NARR.DESC = NARRATIVE
        END ELSE
          CALL SLV.NARR.TXN.INTERNACIONAL(ACCT.NO, Y.FT.ID,Y.TXN.CODE ,NARRATIVE, FLAG.CLIENTE)
                Y.FLAG.CLIENTE = FLAG.CLIENTE
                Y.TRANS.NARR.DESC = NARRATIVE
         END        
    END      
;*----------------------------------------------------------------------------------------------------------------------------         
*wrivas: correccion para pago anticipado
	    ...
	    Y.LF.AMOUNT = R.FUNDS.TRANSFER<FT.LOCAL.REF><1, LF.AMOUNT>
		...
		FIND 'PAGO.X.FT' IN R.KEYS<EB.SLV18.PARAM.ID> SETTING Ap, Vp THEN
			...
			FIND Y.FT.TXN.TYPE IN Y.VALUES SETTING Xp, Yp THEN
				IF Y.LF.AMOUNT THEN  ;*wrivas Monto debito para transacciones de pago anticipado
					;*Asignar Monto a Campo de Estado de Cuenta
					IF Y.STMT.REVERSAL.MARKER EQ 'R' THEN 
						Y.DEBIT.AMOUNT    = R.FUNDS.TRANSFER<FT.LOCAL.REF><1, LF.AMOUNT>
						Y.STMT.AMOUNT.LCY = Y.DEBIT.AMOUNT
					END
					ELSE
						Y.DEBIT.AMOUNT    = R.FUNDS.TRANSFER<FT.LOCAL.REF><1, LF.AMOUNT>
						Y.STMT.AMOUNT.LCY = Y.DEBIT.AMOUNT * -1
					END
				END
				ELSE 
					;*Asignar Monto Credito para transacciones antes de desarrollo de pago anticipado
					IF Y.CREDIT.AMOUNT THEN
						Y.DEBIT.AMOUNT = Y.CREDIT.AMOUNT
					END
...
*----------------------------------------------------------------------------------------------------------------------------	    
       IF Y.STMT.REVERSAL.MARKER EQ 'R' THEN 
       		Y.CREDIT.AMOUNT = Y.CREDIT.AMOUNT
       END
       ELSE
       ...
       ;* CRT Y.DEBIT.AMOUNT
        ...
        ;*@Author: Ronald Ortiz
        ;*@Date: 20190225
        IF Y.FT.TXN.TYPE EQ TRX.CONVENIOS THEN
          Y.TRANS.NARR.DESC = 'Abono a Cuenta'
          NARR              = R.FUNDS.TRANSFER<FT.ORDERING.BANK>  
        END           
...
;*@Author: Ronald Ortiz
;*@Date  : 20190206
;*@Desc  : Se agrega descripción solicitada en el ticket #37783 Abono Específicos <Codigo de Banco><Codigo de Agencia>
        IF Y.FT.TXN.TYPE EQ ABONO.COLECTOR AND R.FUNDS.TRANSFER<FT.ORDERING.CUST> EQ 'ABONO DIARIO A CLIENTE ESP' THEN
               Y.TRANS.NARR.DESC = 'Abono Especificos 41':RIGHT(R.FUNDS.TRANSFER<FT.DEBIT.ACCT.NO>,3)
               NARR = ''
        END

;*@Author: Ronald Ortiz
;*@Date  : 20190206
;*@Desc  : Se agrega descripción solicitada en el ticket #37783 Abono NPE <Codigo de Banco><Codigo de Agencia>
        IF Y.FT.TXN.TYPE EQ ABONO.COLECTOR AND R.FUNDS.TRANSFER<FT.ORDERING.CUST> EQ 'ABONO DIARIO A CLIENTE NPE' THEN
               Y.TRANS.NARR.DESC = 'Abono NPE 41':RIGHT(R.FUNDS.TRANSFER<FT.DEBIT.ACCT.NO>,3)
               NARR = ''
        END
        
;*@Author: Ronald Ortiz
;*@Date  : 20190212        
;*@Desc  : Se agrega descripción solicitada en el ticket #37783 Liquidación de inconsistencias
       IF Y.FT.TXN.TYPE EQ 'AC' AND  R.FUNDS.TRANSFER<FT.ORDERING.BANK> EQ 'SLV.LIQUIDACION.INCON.MH' THEN
               Y.TRANS.NARR.DESC = 'Liquidacion de inconsistencias' 
               NARR = ''
       END
...
;*@Author:Ronald Ortiz
    ...
    CODIGO.COLECTOR.PX = R.FUNDS.TRANSFER<FT.LOCAL.REF, POS.CL.PEX>
	 ...
	 ;*****************************************************************
	 ;* Descripcion moneda azul MAPE
	 ;*****************************************************************
     CALL GET.LOC.REF('FUNDS.TRANSFER','LF.ID.COL',POS.ID.COL)
	 CODIGO.COL.MAPE = R.FUNDS.TRANSFER<FT.LOCAL.REF, POS.ID.COL>
	 IF (Y.FT.TXN.TYPE EQ 'AC' OR Y.FT.TXN.TYPE EQ 'ACAC')AND CODIGO.COL.MAPE EQ 'MAPE' THEN
	 	CALL GET.LOC.REF('FUNDS.TRANSFER','LF.COLECTOR.COD',COLECTOR.COD)
	 	COLECTOR.COD.MAPE = FIELD(R.FUNDS.TRANSFER<FT.LOCAL.REF, COLECTOR.COD>,'_',1)
	 	IF (COLECTOR.COD.MAPE) THEN 
	 		CALL F.READ(FN.SLV.CANAL.MAPE,COLECTOR.COD.MAPE,R.SLV.CANAL.MAPE,F.SLV.CANAL.MAPE,E.SLV.CANAL.MAPE)
       	 	Y.TRANS.NARR.DESC = R.SLV.CANAL.MAPE<EB.SLV96.RESERVADO.3>
         	NARR = ''
	 	END
		ELSE
			Y.TRANS.NARR.DESC = 'Retiro por MonedAzul' 
         	NARR = ''
		END
     END
...
;*wrivas: TXN desde TCIB para Tarjeta Visa
    ...
    Y.TARJETA	= R.FUNDS.TRANSFER<FT.LOCAL.REF, POS.LF.NUMTC>	;*Numero de Tarjeta de credito
		
;* Obteniendo los valores
*	Y.TARJETA = R.FUNDS.TRANSFER<FT.LOCAL.REF,Y.NUMTC>	 
*	Y.TIPOTJ = R.FUNDS.TRANSFER<FT.LOCAL.REF,Y.TIPOTC>
*-------------------------------------------------------------------	
    IF Y.INWARD.PAY.TYPE THEN ;*Proviene de Bulk Payments
        GOSUB GET.FT.BULK.MASTER ;*LF.TXN.BP con salida
    END
    
    ;*Obtener campo local para validar tipo ACH
	CALL GET.LOC.REF('FUNDS.TRANSFER','LF.ID.COL',Y.ID.COL)
    COLECTOR.ACH = R.FUNDS.TRANSFER<FT.LOCAL.REF><1,Y.ID.COL>
    
    IF (LF.TCE.NARR OR LF.TXN.BP) AND Y.FT.TXN.TYPE NE 'AC64' THEN
        IF Y.DEBIT.AMOUNT EQ Y.STMT.AMOUNT.LCY OR LEFT(COLECTOR.ACH,3) EQ 'ACH' THEN
...
* <20191209>        
    	IF Y.CUS.STS EQ '5' AND Y.CUS.SEG = '1' THEN
			ID.RECORD.FOOTER = 'FOOTER.COLLECTOR.PNAE' ;* PNAE
		END
		ELSE 
	        ID.RECORD.FOOTER = 'FOOTER.COLLECTOR.TCE' ;* CORPORATE 
		END
* </20191209>
...
UPDATE.DESCR.CHG:
    ...
    IF Y.APP EQ 'CHG' THEN
        ...
        ELSE
        	FIND Y.TYPE.COMMISSION IN Y.CAT.DIV.CHG SETTING Ap,Vp THEN
	        ;*IF Y.TYPE.COMMISSION EQ 'COMMER' OR Y.TYPE.COMMISSION EQ 'MNDAZULMAS' OR Y.TYPE.COMMISSION EQ 'TRANSFINT' THEN
...
* <20191209>        
    	IF Y.CUS.STS EQ '5' AND Y.CUS.SEG = '1' THEN
			ID.KEYS.PARAMS 	= 'SUB.TXN.TYPE.PNAE' ;* PNAE
		END
		ELSE 
	        ID.KEYS.PARAMS 	= 'SUB.TXN.TYPE' ;*id con Narratives parametrizados en EB.KEYS.PARAMS	- CORPORATE 
		END
* </20191209>		
...
*---------------------------------------------------------------------------------------------------------------------------
					IF Y.TARJETA THEN	
						Y.TRANS.NARR.DESC = R.FT.TXN.TYPE.CONDITION<FT6.DESCRIPTION><1,1> : ' ' :  REC.SUBTYPE.TXN<EB.SLV18.VALOR><1,POS.SUB.TYPE2> : Y.TARJETA				
					END ELSE
...
;*Segmento para Ocultar Desvio de Seguro a Cuenta Transitoria por Pago Anticipado : OCORNEJO 06.03.2018
;*=====================================================================================================
HIDE.UNC.TXN:
	;*Extraer FT Originadora del Movimiento
	Y.ALP.REF = R.FUNDS.TRANSFER<FT.CREDIT.THEIR.REF>
	
	;*Buscar la FT para Identificar si es ALP
	CALL F.READ(FN.FUNDS.TRANSFER, Y.ALP.REF, R.FT.ALP, F.FUNDS.TRANSFER, E.FT.ALP)
	IF R.FT.ALP EQ '' THEN
		;*elara - 2020-01-09 - Se agrega el sufijo $HIS a la aplicación
		CALL EB.READ.HISTORY.REC(F.FUNDS.TRANSFER$HIS, Y.ALP.REF, R.FT.ALP, E.FT.ALP)
	END
	
	Y.FLG.HIDE = 'N'	
	IF LEFT(R.FT.ALP<FT.CREDIT.THEIR.REF>,3) EQ 'ALP' OR LEFT(R.FT.ALP<FT.DEBIT.THEIR.REF>,3) EQ 'ALP' THEN
		Y.FLG.HIDE = 'Y'
	END 
RETURN
;*=====================================================================================================
*** <region name= GET.NARRATIVE.CM>
*** <desc>Obtiene la narrativa para los colectores manuales </desc>
;*@Author: rmezquita
;*@Date: 20180823 
GET.NARRATIVE.CM:	
	COD.COLECTOR = R.TFS<TFS.LOCAL.REF><1,FLD.POS>
	...
	AGENCIA = ''
	IF COD.COLECTOR EQ '04' OR COD.COLECTOR EQ '05' OR COD.COLECTOR EQ '08' THEN
		COD.COMPANY = R.TFS<TFS.CO.CODE>
     	CALL F.READ (FN.COMPANY,COD.COMPANY,R.COMP,F.COMPANY,ER)
    	AGENCIA = R.COMP<EB.COM.COMPANY.NAME>
	END
    ;*COD.COMPANY = Y.STMT.COMPANY
    
	IF COD.COLECTOR THEN  
		RECORD.ABONO.APP = 'SLV.COLECTORES.DESC'
    	CALL F.READ(FN.EB.SLV.KEYS.PARAMS,RECORD.ABONO.APP,RESPONSE.NRR.ABN,F.EB.SLV.KEYS.PARAMS,ERR.APP.KEYS)
    	
    	CALL F.READ(FN.SLV.COLECTORES, COD.COLECTOR, RECORD.COLECTOR, F.SLV.COLECTORES, ERROR.COLECTORES)    	
		V.PREFI.COLECTOR 	= RECORD.COLECTOR<EB.CL.CMP.RESERVA.10>  
		V.NOMBRE.COLECTOR 	= RECORD.COLECTOR<EB.CL.NOMBRE.COLECTOR>
		
		R.FIND = CM.COLECTOR:'.':COD.COLECTOR	
		
    	NARRATIVA.COLEC = 'Por Pago de Colector'
		FINDSTR R.FIND IN RESPONSE.NRR.ABN<EB.SLV18.PARAM.ID> SETTING Ap, Vp THEN
			NARRATIVA.COLEC = ''
			NARRATIVA.COLEC = RESPONSE.NRR.ABN<EB.SLV18.VALOR><1, Vp>	
			IF COD.COLECTOR EQ '04' OR COD.COLECTOR EQ '05' OR COD.COLECTOR EQ '08' THEN
				NARRATIVA.COLEC=NARRATIVA.COLEC:VM:AGENCIA	
			END
		END
		Y.TRANS.NARR.DESC = NARRATIVA.COLEC
	END  
